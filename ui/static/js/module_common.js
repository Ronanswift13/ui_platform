...
document.addEventListener('DOMContentLoaded', function() {
    const MODULES = {
        ...
        // ========== 新增模块 ==========
        bird: { pluginId: 'bird_monitoring' },
        indoor_fence: { pluginId: 'indoor_fence' },
    };
    ...
    function buildAlgorithmsConfig() {
        ...
        if (moduleId === 'bird') {
            return {
                bird_detection: {
                    enabled: isChecked('check-bird-detection', true),
                    confidence_threshold: readNumber('bird-confidence', 0.5),
                    tracking_enabled: true,
                    max_track_age: readInt('bird-max-track-age', 30),
                },
                species_identification: {
                    enabled: isChecked('check-species-id', true),
                },
                risk_assessment: {
                    enabled: isChecked('check-risk-assessment', true),
                    safe_distance_m: readNumber('bird-safe-distance', 15),
                    warning_distance_m: readNumber('bird-warning-distance', 10),
                    danger_distance_m: readNumber('bird-danger-distance', 5),
                },
                nest_detection: {
                    enabled: isChecked('check-nest-detection', false),
                },
                deterrent: {
                    enabled: isChecked('check-deterrent-enabled', false),
                    acoustic_enabled: isChecked('check-acoustic', true),
                    laser_enabled: isChecked('check-laser', false),
                    auto_trigger_level: document.getElementById('deterrent-trigger-level')?.value || 'danger',
                },
            };
        }

        if (moduleId === 'indoor_fence') {
            const authCabinets = [];
            document.querySelectorAll('.cabinet-auth-btn.btn-success').forEach(btn => {
                authCabinets.push(parseInt(btn.dataset.cabinetId));
            });

            return {
                person_detection: {
                    enabled: isChecked('check-person-detection', true),
                    confidence_threshold: readNumber('person-confidence', 0.5),
                    max_persons: readInt('max-persons', 10),
                },
                line_check: {
                    enabled: isChecked('check-line-detection', true),
                    yellow_line_distance_m: readNumber('yellow-line-distance', 0.5),
                    warning_distance_m: readNumber('warning-distance', 0.3),
                    danger_distance_m: readNumber('danger-distance', 0.1),
                },
                authorization: {
                    enabled: isChecked('check-auth', true),
                    allow_list: authCabinets,
                },
                multi_person: {
                    enabled: isChecked('check-multi-person', true),
                    max_per_cabinet: readInt('max-per-cabinet', 1),
                },
            };
        }

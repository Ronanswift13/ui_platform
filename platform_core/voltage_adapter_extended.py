...
class VoltageAdapterManager:
    ...
    def __init__(self, config_path: str = "configs/voltage_config.yaml"):
        ...
        self.current_level_display: Optional[str] = None  # 保存前端显示格式
        ...
        if self.config_path.exists():
            try:
                with open(self.config_path, 'r', encoding='utf-8') as f:
                    ...
                    if config:
                        # 优先使用前端显示格式，否则使用标准化格式
                        level = config.get("current_voltage_level_display") or config.get("current_voltage_level")
                        if level:
                            self.set_voltage_level(level)
    ...
    def _save_config(self):
        ...
        try:
            ...
            config = {
                ...
                "current_voltage_level_display": self.current_level_display,  # 保存前端格式
    ...
    def set_voltage_level(self, level: str) -> bool:
        ...
        try:
            ...
            # 保存显示格式：如果是前端格式则保存，否则反向查找前端格式
            self.current_level_display = self._get_display_level(original_level, normalized_level)
    ...
    def _normalize_voltage_level(self, level: str) -> str:
        ...
        aliases = {
            ...
            "UHV_800kV_DC": "±800kV_DC",
            "UHV_1100kV_DC": "±1100kV_DC",
            "EHV_500kV": "500kV_AC",
            "EHV_330kV": "330kV_AC",
            "EHV_750kV": "750kV_AC",
            "EHV_500kV_DC": "±500kV_DC",
            "HV_220kV": "220kV",
            "HV_110kV": "110kV",
            "MV_35kV": "35kV",
            "MV_66kV": "66kV",
    ...
    def _get_display_level(self, original_level: str, normalized_level: str) -> str:
        """获取前端显示格式的电压等级"""
        # 前端格式列表
        frontend_formats = {
            "UHV_1000kV_AC", "UHV_800kV_DC", "UHV_1100kV_DC",
            "EHV_500kV", "EHV_330kV", "EHV_750kV", "EHV_500kV_DC",
            "HV_220kV", "HV_110kV",
            "MV_35kV", "MV_66kV",
        ...
        # 如果原始输入就是前端格式，直接返回
        if original_level in frontend_formats:
            return original_level

        # 反向映射：后端格式 -> 前端格式
        reverse_mapping = {
            "1000kV_AC": "UHV_1000kV_AC",
            "±800kV_DC": "UHV_800kV_DC",
            "±1100kV_DC": "UHV_1100kV_DC",
            "500kV_AC": "EHV_500kV",
            "500kV": "EHV_500kV",
            "330kV_AC": "EHV_330kV",
            "330kV": "EHV_330kV",
            "750kV_AC": "EHV_750kV",
            "±500kV_DC": "EHV_500kV_DC",
            "220kV": "HV_220kV",
            "110kV": "HV_110kV",
            "35kV": "MV_35kV",
            "66kV": "MV_66kV",
        ...
        return reverse_mapping.get(normalized_level, original_level)
    ...
    def get_current_level(self) -> Optional[str]:
        """获取当前电压等级（返回前端显示格式）"""
        # 优先返回前端显示格式，如果没有则返回标准化格式
        return self.current_level_display or self.current_level

    def get_current_level_internal(self) -> Optional[str]:
        """获取当前电压等级（返回内部标准化格式，用于配置匹配）"""
    ...
    def get_voltage_level_info(self) -> Dict[str, Any]:
        """获取当前电压等级的完整信息"""
        return {
            "voltage_level": self.get_current_level(),  # 返回前端格式
            "voltage_level_internal": self.current_level,  # 内部标准化格式

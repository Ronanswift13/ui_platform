#!/usr/bin/env bash
# Fast module-scoped tests for switch_inspection.
...
Modules:
  ...
  detector     detector contract and fallback chain
  consistency  switch_consistency logic tests
  config       config and reason-code contract
  quality      governance asset contract
...
case "$MODULE" in
  ...
  detector)
    ...
    ;;
  consistency)
    TEST_FILES=("tests/test_switch_consistency.py")
    ;;
  config)
    TEST_FILES=("tests/test_config_contract.py" "tests/test_reason_code_contract.py")
    ;;
  quality)
    TEST_FILES=("tests/test_quality_gate_contract.py")
    ;;
  all)
    TEST_FILES=("tests")
    ;;
...
echo "========================================"
echo "Targeted Tests - switch_inspection"
echo "module=$MODULE"

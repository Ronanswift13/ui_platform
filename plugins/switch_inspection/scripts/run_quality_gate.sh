#!/usr/bin/env bash
# Governance + architecture quality gate for switch_inspection.
...
echo "========================================"
echo "Quality Gate - switch_inspection"
echo "========================================"
...
if rg -n "except\s*:|except\s+Exception\s*:\s*pass" plugin.py detector_enhanced.py switch_consistency.py >/dev/null 2>&1; then
  echo "[FAIL] Silent exception anti-pattern detected"
  rg -n "except\s*:|except\s+Exception\s*:\s*pass" plugin.py detector_enhanced.py switch_consistency.py
  exit 1
fi

if rg -n "\bprint\(" plugin.py detector_enhanced.py switch_consistency.py >/dev/null 2>&1; then
  echo "[FAIL] print() found in production modules"
  rg -n "\bprint\(" plugin.py detector_enhanced.py switch_consistency.py
  exit 1
fi
...
if rg -n "requests\.|http://|https://|urllib|aiohttp" \
  plugin.py detector_enhanced.py switch_consistency.py standalone scripts \
  --glob '!scripts/run_quality_gate.sh' >/dev/null 2>&1; then
  echo "[FAIL] Network call detected"
  rg -n "requests\.|http://|https://|urllib|aiohttp" \
    plugin.py detector_enhanced.py switch_consistency.py standalone scripts \
    --glob '!scripts/run_quality_gate.sh'
  exit 1
fi

if rg -n "rm -rf|sudo |os\.remove\(|shutil\.rmtree\(" scripts \
  --glob '!scripts/run_quality_gate.sh' >/dev/null 2>&1; then

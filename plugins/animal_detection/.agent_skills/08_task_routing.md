...
2. **最小实现** — 不跨越层级边界（core/ 不依赖 SDK，plugin.py 不含算法逻辑）
...
| 实现模块 (`plugin.py` / `core/*.py`) | 必须：最小改动 |
...
| 5 | `.agent_skills/09_runtime_dependency_repair.md` | 若为运行时依赖问题 |
...
1. **影响评估** — 列出受影响的模块与测试
2. **先更新测试** — 按新契约更新测试期望值
3. **适配实现** — 按层级从底层（core/）到顶层（plugin.py）逐层适配

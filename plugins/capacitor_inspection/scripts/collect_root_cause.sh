#!/usr/bin/env bash
# Collect minimum reproducibility materials for capacitor_inspection failures.
...
if git -C "$REPO_ROOT" rev-parse --is-inside-work-tree >/dev/null 2>&1; then
  git -C "$REPO_ROOT" status --short -- plugins/capacitor_inspection > "$CASE_DIR/git_status.txt" || true
  git -C "$REPO_ROOT" diff -- plugins/capacitor_inspection > "$CASE_DIR/git_diff.patch" || true
fi

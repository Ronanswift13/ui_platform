#!/usr/bin/env bash
# Regression gate for capacitor_inspection.
...
echo "========================================"
echo "Regression Gate - capacitor_inspection"
echo "========================================"
...
echo "[NOTE] Intrusion positive cases are validated by contract tests, not by a replay corpus."
...
if "$PYTHON_CMD" -m pytest --help 2>/dev/null | grep -q -- "--cov"; then
  "$PYTHON_CMD" -m pytest tests/ -q \
    --cov=plugins.capacitor_inspection \

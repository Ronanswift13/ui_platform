...
All `/implement`, `/repair`, and `/audit` work for `capacitor_inspection` must read this file first.
...
1. decide the target module: `standalone|plugin|detector|config|quality`
...
4. do not claim replay coverage, field accuracy, or stable intrusion effectiveness that is not present

...
class TestImportWeight:
    """验证 capacitor_inspection 模块导入不触发 torch/training 初始化。"""

    def test_plugin_module_import_no_torch(self):
        """import plugin.py 不应加载 torch（延迟到 init）。"""
        mods_to_remove = [
            k for k in sys.modules
            if k.startswith("plugins.capacitor_inspection.plugin")
        ]
        ...
        import importlib
        mod = importlib.import_module("plugins.capacitor_inspection.plugin")

        assert hasattr(mod, "Plugin")
        assert hasattr(mod, "CapacitorInspectionPlugin")
        assert hasattr(mod, "_get_model_resolver")
        assert callable(mod._get_model_resolver)

    def test_package_init_import_no_torch(self):
        """import plugins.capacitor_inspection 包级别不应加载 plugin.py。"""
        if "torch" in sys.modules:
            pytest.skip("torch already loaded by prior test; run in isolation")

        mods_to_remove = [
            k for k in sys.modules
            if k.startswith("plugins.capacitor_inspection")
        ]
        for m in mods_to_remove:
            del sys.modules[m]

        import importlib
        importlib.import_module("plugins.capacitor_inspection")

        assert "plugins.capacitor_inspection.plugin" not in sys.modules

    def test_create_standalone_succeeds(self):
        """create_standalone() 完整链路仍可用。"""
        from plugins.capacitor_inspection.plugin import Plugin
    ...
    def test_lazy_getattr_works(self):
        ...
        import plugins.capacitor_inspection as pkg

        CapPlugin = pkg.CapacitorInspectionPlugin
        assert CapPlugin is not None
        assert hasattr(CapPlugin, "create_standalone")

#!/usr/bin/env bash
# Fast module-scoped tests for temperature_monitoring.
...
Modules:
  ...
  plugin       plugin contract tests
  config       config and manifest contract
...
case "$MODULE" in
  standalone)
    ...
    ;;
  plugin)
    TEST_FILES=("tests/test_plugin_contract.py" "tests/test_plugin_detect.py")
    ;;
  config)
...
echo "========================================"
echo "Targeted Tests - temperature_monitoring"
echo "module=$MODULE"

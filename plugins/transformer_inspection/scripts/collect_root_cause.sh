#!/usr/bin/env bash
# Collect minimum reproducibility materials for transformer_inspection failures.
...
cp -f manifest.json "$CASE_DIR/manifest.json" 2>/dev/null || true
...
if git -C "$REPO_ROOT" rev-parse --is-inside-work-tree >/dev/null 2>&1; then
  git -C "$REPO_ROOT" status --short -- plugins/transformer_inspection > "$CASE_DIR/git_status.txt" || true
  git -C "$REPO_ROOT" diff -- plugins/transformer_inspection > "$CASE_DIR/git_diff.patch" || true
fi

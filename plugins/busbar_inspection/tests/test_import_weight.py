...
class TestImportWeight:
    """验证 busbar_inspection 模块导入不触发 torch/training 初始化。"""
    ...
    def test_plugin_module_import_no_torch(self):
        ...
        mods_to_remove = [
            ...
            if k.startswith("plugins.busbar_inspection.plugin")
        ]
        for m in mods_to_remove:
            del sys.modules[m]

        # 检查当前 torch 状态（若此前其他测试已加载 torch 则 skip）
        if "torch" in sys.modules:
            pytest.skip("torch already loaded by prior test; run in isolation")

        # 重新 import
        import importlib
        mod = importlib.import_module("plugins.busbar_inspection.plugin")

        # 验证 Plugin 类存在
        assert hasattr(mod, "Plugin")
        assert hasattr(mod, "BusbarInspectionPlugin")

        # 验证 _get_model_resolver 存在（延迟加载函数）
        assert hasattr(mod, "_get_model_resolver")
        assert callable(mod._get_model_resolver)

    def test_package_init_import_no_torch(self):
        """import plugins.busbar_inspection 包级别不应加载 plugin.py。"""
        if "torch" in sys.modules:
            pytest.skip("torch already loaded by prior test; run in isolation")

        mods_to_remove = [
            k for k in sys.modules
            if k.startswith("plugins.busbar_inspection")
        ]
        for m in mods_to_remove:
            del sys.modules[m]

        import importlib
        importlib.import_module("plugins.busbar_inspection")

        # plugin.py 不应被加载
        assert "plugins.busbar_inspection.plugin" not in sys.modules
    ...
    def test_lazy_getattr_works(self):
        ...
        import plugins.busbar_inspection as pkg

        # 通过 __getattr__ 延迟访问
        BusbarPlugin = pkg.BusbarInspectionPlugin
        assert BusbarPlugin is not None
        assert hasattr(BusbarPlugin, "create_standalone")
